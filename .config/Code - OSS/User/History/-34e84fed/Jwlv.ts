...
import { arradd, arrremove, css, scss, assetsDir, dark, themedir,SCREEN_WIDTH, SCREEN_HEIGHT} from "./util.ts";
import { Workspaces } from "./widgets/workspace.ts";
import { NierBorder } from "./widgets/nier_border.ts";
import { button_pointer_size, top_icon_size, top_spacing, workspace_height, workspace_width } from "./scaling.ts";
...
execAsync(`ags -b player -c ${App.configDir}/windows/player/player.ts`);
execAsync(`ags -b settings -c ${App.configDir}/windows/settings/settings.ts`);
...
execAsync(["bash","-c",`pkill dunst;ags -b notify -c ${App.configDir}/windows/notifications/notifications.ts`])
